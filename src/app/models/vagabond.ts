...
${canBeModified && vaga.hasTrait('Adventurer') ? translate.instant('SpecificDaylight.Vagabot.ActionQuestRepeat') : ''}`;
      ...
      aid() {
        ...
        return translate.instant('SpecificDaylight.Vagabot.ActionAid', {
          aidHelpText,
        });
      },
      ...
      battle() {
        const target = vaga.hasTrait('Berserker')
          ? translate.instant(
              'SpecificDaylight.Vagabot.ActionBattleTargetBerserker',
            )
...
${translate.instant('SpecificExtra.Vagabot.Special' + vaga.customData.chosenVaga)}
  ...
  public daylight(translate: TranslateService) {
    ...
    if (this.hasTrait('Adventurer')) {
      base.push(this.createMetaData('score', 1, actions.quest(true)));
    }
  ...
  public evening(translate: TranslateService) {
    ...
    const base = [
      this.createMetaData(
        'text',
        '',
        translate.instant('SpecificEvening.Vagabot.Refresh', {
          itemRefreshMin,
          itemRefreshMax,
        }),
      ),
    ...
    if (this.difficulty === 'Nightmare') {
      base.push(
        this.createMetaData(
          'score',
          1,
          translate.instant('SpecificEvening.Vagabot.NightmareScore'),
        ),
      );
    }
